...
@section('seo_title')Receipt {{ $order->order_number }} — Sunset Bagel Exchange@endsection

@section('content')
    <div class="container py-4" style="max-width: 42rem;">
        <div class="text-end mb-3 no-print">
            <button class="btn btn-brand" onclick="window.print()"><i class="bi bi-printer me-1" aria-hidden="true"></i>Print</button>
        </div>

        <div class="card receipt-card">
            <div class="card-body p-4">
                <div class="text-center mb-4">
                    <h1 class="display-heading h3 text-burgundy mb-0">Sunset Bagel Exchange</h1>
                ...
                <table class="table table-sm">
                    <thead>
                        <tr><th scope="col">Item</th><th scope="col" class="text-center">Qty</th><th scope="col" class="text-end">Total</th></tr>
                    </thead>
                    <tbody>
                    ...
                    <tfoot>
                        ...
                        @if((float) $order->discount > 0)
                            <tr><td colspan="2" class="text-end">Discount @if($order->coupon_code)({{ $order->coupon_code }})@endif</td><td class="text-end">−${{ number_format((float) $order->discount, 2) }}</td></tr>
                        @endif
                ...
                @if($order->notes)
                    <p class="small mb-0"><strong>Notes:</strong> {{ $order->notes }}</p>
                @endif
                <p class="text-center small text-body-secondary mt-4 mb-0">Thank you for supporting your local bagel shop! 🥯</p>
            </div>
        </div>
    </div>
@endsection
